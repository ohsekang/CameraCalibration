...
| -bw        | int  | 7         | Chess Board Width (corners number)               | 보드 너비. [안쪽 모서리 수 (사각형 수 + 1)]             |
| -bh        | int  | 6         | Chess Board Height (corners number)              | 보드의 높이. [안쪽 모서리 수(사각형 수 + 1)]             |
...
#### 예 2(오프라인 보정 = 데이터 기반 보정)    
(참고: **동영상 또는 이미지를 . /data/에 저장하고 기본값**에 따라 이름을 지정하면 쉽게 사용할 수 있도록 매개변수 입력을 저장할 수 있습니다.) 

오프라인으로 보정하는 경우 입력은 로컬 비디오이고 파일은 . /data/video.mp4에 있고, 바둑판 모서리 점의 수가 6×8이며, 기본값을 유지하기 위해 매개변수를 입력하지 않은 경우 다음을 입력합니다.  
```
